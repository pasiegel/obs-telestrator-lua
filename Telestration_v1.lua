-- OBS Telestration Script
-- Authors: Mike Welsh (mwelsh@gmail.com), Tari, Paladin
-- v1.5
...
source_def.create = function(source, settings)
    ...
    if obs.obs_get_video_info(video_info) then
        ...
        create_telestration_texture(data)
    else
...
function script_load(settings)
    ...
    local hotkey_save_array = obs.obs_data_get_array(settings, "telestration.clear")
    obs.obs_hotkey_load(hotkey_clear, hotkey_save_array)
    obs.obs_data_array_release(hotkey_save_array)
    
    hotkey_color = obs.obs_hotkey_register_frontend("telestration.colorswap", "Swap Telestrator Color", colorswap)
    if hotkey_color == nil then
        hotkey_color = obs.OBS_INVALID_HOTKEY_ID
    end
    local hotkey_save_array2 = obs.obs_data_get_array(settings, "telestration.colorswap")
    obs.obs_hotkey_load(hotkey_color, hotkey_save_array2)
    obs.obs_data_array_release(hotkey_save_array2)
    
    hotkey_size = obs.obs_hotkey_register_frontend("telestration.sizetoggle", "Toggle Telestrator Pencil Size", sizetoggle)
    if hotkey_size == nil then
        hotkey_size = obs.OBS_INVALID_HOTKEY_ID
    end
    local hotkey_save_array3 = obs.obs_data_get_array(settings, "telestration.sizetoggle")
    obs.obs_hotkey_load(hotkey_size, hotkey_save_array3)
    obs.obs_data_array_release(hotkey_save_array3)
    
    hotkey_erase = obs.obs_hotkey_register_frontend("telestration.erasertoggle", "Toggle Telestrator Eraser", erasertoggle)
...
function script_save(settings)
    ...
    obs.obs_data_set_array(settings, "telestration.clear", hotkey_save_array)
    obs.obs_data_array_release(hotkey_save_array)
    
    hotkey_save_array = obs.obs_hotkey_save(hotkey_color)
    obs.obs_data_set_array(settings, "telestration.colorswap", hotkey_save_array)
    obs.obs_data_array_release(hotkey_save_array)
    
    hotkey_save_array = obs.obs_hotkey_save(hotkey_size)
    obs.obs_data_set_array(settings, "telestration.sizetoggle", hotkey_save_array)
    obs.obs_data_array_release(hotkey_save_array)
    
    hotkey_save_array = obs.obs_hotkey_save(hotkey_erase)
    obs.obs_data_set_array(settings, "telestration.erasertoggle", hotkey_save_array)
    obs.obs_data_array_release(hotkey_save_array)
end
...
function create_telestration_texture(data)
    obs.obs_enter_graphics()
    ...
    print("create telestration texture " .. data.width .. " " .. data.height)
...
function script_description()
    ...
    return [==[Adds telestration ability in obs.
    
Add the Telestrator source on top of your scene, then project your entire scene and draw on the projector window. Each scene can have one telestration.
